const manageBtn = document.querySelector('.colltection-title button');  //관리하기 버튼
...
function handleManageBtn() {
  const ClassNames = manageBtn.classList;
  ClassNames.toggle('manage');
  manageBtn.innerHTML = ClassNames.contains('manage') ? '돌아가기' : '관리하기';
  handleDelGroup(ClassNames.contains('manage'));
}


manageBtn.addEventListener('click', handleManageBtn);
